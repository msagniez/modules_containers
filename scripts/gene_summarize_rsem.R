...
#gene_summarize_rsem.R - Summarize RSEM transcript quantifications to gene level

#Parse command line arguments
args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 2) {
  cat("Usage: Rscript gene_summarize_rsem.R [--gene-names] [--ignore-tx-version] <gtf_file> <isoforms_file1> [isoforms_file2 ...]\n")
  cat("   or: Rscript gene_summarize_rsem.R [--gene-names] [--ignore-tx-version] --tx2gene <tx2gene.txt> <isoforms_file1> [isoforms_file2 ...]\n")
  cat("\nOptions:\n")
  ...
  cat("\nInput: RSEM *.isoforms.results files (tab-delimited, with header:\n")
  cat("       transcript_id, gene_id, length, effective_length, expected_count, TPM, FPKM, IsoPct)\n")
...
#(RSEM isoforms.results files have a header row: transcript_id, gene_id, length,
# effective_length, expected_count, TPM, FPKM, IsoPct)
first_quant <- read.table(quant_files[1], header = TRUE, nrow = 100, sep = "\t")
...
#Strips everything from the first "." onward, so "sample1.isoforms.results" -> "sample1"
names(quant_files) <- gsub(".*/", "", gsub("\\..*$", "", quant_files))
cat("Importing", length(quant_files), "RSEM isoforms.results file(s)...\n")

# ignoreTxVersion = TRUE only when flag set AND versions are on the quant file side
txi <- tximport(quant_files, type = "rsem", tx2gene = tx2gene, ignoreTxVersion = ignore_tx_version)
